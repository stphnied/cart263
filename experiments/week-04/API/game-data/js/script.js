...
function setup() {
  ...
  let data = JSON.parse(localStorage.getItem(`click-game-data`));
...
function mousePressed() {
  ...
  if (clicks > gameData.highScore) {
    ...
    localStorage.setItem(`click-game-data`, JSON.stringify(gameData));
  }
}

function keyPressed() {
    if(key === `c`) {
        localStorage.removeItem(`click-game-data`);
    }
}